...
public class Move_telescope : MonoBehaviour
{
...

	{
		...
		while (Input.GetMouseButton(0))
		{
			...
			// 更新目标物体的世界空间坐标 
			if (_vec3TargetWorldSpace.y > 55f)
            {
				_vec3TargetWorldSpace.y = 55f;

			}
			if (_vec3TargetWorldSpace.y < mirror.transform.position.y + 9f)
            {
				_vec3TargetWorldSpace.y = mirror.transform.position.y + 9f;
